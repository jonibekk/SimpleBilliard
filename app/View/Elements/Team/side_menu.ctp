...
<ul class="nav" style="font-size: 13px;">
    <li class=""><a <?php if ($angular): ?>ui-sref="member"
                    <?php else: ?>href="<?= $this->Html->url(['controller' => 'teams', 'action' => 'main', '#' => '/']) ?>"<?php endif ?>><i
                class="fa fa-user"></i> <?= __('Team Members') ?></a></li>
    <li class=""><a <?php if ($angular): ?>ui-sref="vision({team_id:team_id})"
                    <?php else: ?>href="<?= $this->Html->url(['controller' => 'teams', 'action' => 'main', '#' => '/vision/' . $this->Session->read('current_team_id')]) ?>" <?php endif ?>><i
                class="fa fa-rocket"></i> <?= __('Team Visions') ?></a></li>
    <li class=""><a <?php if ($angular): ?>ui-sref="group_vision({team_id:team_id})"
                    <?php else: ?>href="<?= $this->Html->url(['controller' => 'teams', 'action' => 'main', '#' => '/group_vision/' . $this->Session->read('current_team_id')]) ?>"<?php endif ?>><i
