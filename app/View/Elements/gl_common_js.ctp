    ...
    var cake = {
        message: {
            validate: {
                ...
                dropzone_file_too_big: "<?=__d('app', 'アップロード出来るファイルサイズは{{maxFilesize}}MBまでです。')?>",
                dropzone_invalid_file_type: "<?=__d('app', '画像ファイルを選択してください。')?>",
                dropzone_max_files_exceeded: "<?=__d('app', 'アップロード出来るファイル数は{{maxFiles}}個までです。')?>",
                dropzone_response_error: "<?=__d('app', 'アップロードに失敗しました。')?>",
                dropzone_cancel_upload: "<?=__d('app', 'アップロードをキャンセルしました。')?>",
                dropzone_cancel_upload_confirmation: "<?=__d('app', 'アップロードをキャンセルしてよろしいですか？')?>",
                dropzone_uploading_not_end: "<?=__d('app', '全てのファイルのアップロードが完了していません。アップロード完了までお待ち下さい。')?>",
                dropzone_uploaded_file_expired: "<?=__d('app', 'アップロードしたファイルの有効期限が切れています。再度アップロードしてください。')?>",
                date_format: "<?=__d('validate',"日付はYYYY/MM/DDの形式で入力してください。")?>",
            },
            notice: {
                a: "<?=__d('app',"入力が途中です。このまま移動しますか？")?>",
                b: "<?=__d('app',"スペルを入力してください。")?>",
                c: "<?=__d('app',"エラーが発生しました。データ取得できません。")?>",
                d: "<?=__d('app',"エラーが発生しました。")?>",
                e: "<?=__d('validate',"開始日が期限を過ぎています。")?>",
                f: "<?=__d('validate',"期限が開始日以前になっています。")?>",
                g: "<?=__d('validate',"コメント送信できませんでした")?>",
                h: "<?=__d('app',"このコメントは削除されました。")?>",
                i: "<?=__d('app',"コメントを取得できませんでした。")?>",
                search_result_zero: "<?=__d('app',"該当なし")?>",
                leave_secret_circle: "<?=__d('app','秘密のサークルから脱退すると、自分の操作で再度参加できません。よろしいですか？')?>"
            },
            info: {
                a: "<?=__d('app',"クリップボードに投稿URLをコピーしました。")?>",
                b: "<?=__d('app',"読込中･･･")?>",
                c: "<?=__d('app',"検索中･･･")?>",
                d: "<?=__d('app',"フォロー中")?>",
                e: "<?=__d('app', "もっと見る") ?> ",
                f: "<?=__d('app', "さらに投稿を読み込む ▼") ?> ",
                g: "<?=__d('app','これ以上のコメントがありません。')?>",
                h: "<?=__d('app',"閉じる")?>",
                z: "<?=__d('app',"フォロー")?>", // ToDo - must rename this var
                copy_url: "<?=__d('app',"以下のURLをコピーしてください。")?>"
            }
        },
        word: {
            a: "<?=__d('app',"自分のみ")?>",
            ...
            h: "<?=__d('app',"文字長すぎます")?>",
            i: "<?=__d('app',"最多で")?>",
            j: "<?=__d('app',"項目までしか選択できません")?>",
            k: "<?=__d('app',"達成要素を選択する(オプション)")?>",
            l: "<?=__d('app',"達成要素はありません")?>",
            select_notify_range: "<?=__d('app',"通知先を追加(オプション)")?>",
            public: "<?=__d('app',"公開")?>",
            secret: "<?=__d('app',"秘密")?>",
            select_public_circle: "<?=__d('app',"公開サークルかメンバーを指定しよう")?>",
            select_public_message: "<?=__d('app',"メンバーを指定しよう")?>",
            select_secret_circle: "<?=__d('app',"秘密サークルを指定しよう")?>",
            share_change_disabled: "<?=__d('app',"サークルページでは切り替えられません")?>",
            success: "<?=__d('app',"成功")?>",
            error: "<?=__d('app',"エラー")?>",
            cancel: "<?=__d('app',"キャンセル")?>",
            search_placeholder_user: "<?=__d('app',"名前を入力")?>",
            search_placeholder_goal: "<?=__d('app',"ゴール名を入力")?>",
            search_placeholder_circle: "<?=__d('app',"サークル名を入力")?>",
            leave_circle: "<?=__d('app',"サークルを脱退する")?>",
            config: "<?=__d('app',"設定")?>"
        },
        url: {
