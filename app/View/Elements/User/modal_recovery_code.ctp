...
<div class="modal-dialog" id="RecoveryCodeModal">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title"><?= __d('app', "リカバリーコード") ?></h4>
        </div>
        <div class="modal-body">
            <div class="two_fa_step_box">
                <?= __d('app', 'これらのコードを印刷するか書き留めて、安全な場所に保管しましょう。各リカバリーコードは１回のみ使用できます。') ?>
            </div>
            <ol id="RecoveryCodeList">
                <?php foreach ($recovery_codes as $v): ?>
                    ...
                    <?php if ($v['RecoveryCode']['used']): ?>
                        <li><span class="font_lightgray"><s><?= h($_code) ?></s>&nbsp;&nbsp;<span class="font_10px"><?= __d('app', "使用済") ?></span></span></li>
                    <?php else: ?>
                        <li><?= h($_code) ?></li>
                    <?php endif ?>
                <?php endforeach ?>
            ...
            <hr>
            <?= $this->Html->link(__d('app', "新しいリカバリーコードを作成"), '#', [
                'class' => 'regenerate-recovery-code'
            ]) ?>
            <div class="font_11px">
                <?= __d('app', '新しいリカバリーコードを生成すると、上記のリカバリーコードは全て無効になります。') ?>
            </div>
