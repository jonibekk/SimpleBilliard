...
株式会社ＩＳＡＯ（以下「ISAO」といいます）と目標達成支援ツール「Goalous」利用者（以下「利用者」といいます）は、Goalous利用規約（以下「原規約」といいます）にもとづき、両者間で成立した「Goalous」利用契約（以下「Goalous利用契約」といいます）について、以下条件にてGoalousを利用することができます。なお、本特約にて使用する用語・定義は特段の定めがある場合を除き、原規約と同一とする。
...
第2条（協力義務）
利用者は、以下に例示するISAOによるGoalousの販売活動に最大限協力するものとします。
(1) Goalousの利用状況、その効果等について、ISAOからの取材に応じる
(2) Goalousサービスサイトや紹介サイト、その他販促ツールへの利用者の商号（ロゴ含む）、代表者及び担当者名、取材内容等の掲載
(3) ISAOが主催するGoalous紹介セミナー等での講演
...
(3) 適用期間満了日より前にGoalousを中途解約した場合、中途解約日から適用期間満了日の残存期間において発生するGoalous利用料（キャンペーン価格適用前）を中途解約日までに一括してISAOに支払います。なお、サービスはその月の請求期間の最後まで続きます。
