...
<section class="panel subscription-status <?= $serviceUseStatus == Goalous\Model\Enum\Team\ServiceUseStatus::PAID ? 'has-subnav' : '';?>">
    ...
    <div class="panel-container">
        ...
        <?php if ($serviceUseStatus == Team::SERVICE_USE_STATUS_FREE_TRIAL): ?>
            <p class="subscription-status-detail"><?= __('Your team is currently using Goalous as a free trial. Your free trial will end on %s', $this->TimeEx->formatYearDayI18nFromDate($team['service_use_state_end_date'])); ?></p>
        <?php elseif ($serviceUseStatus == Team::SERVICE_USE_STATUS_PAID): ?>
            <p class="subscription-status-detail"><?= __('Your team has full access to Goalous.<br /><br />Go achieve your goal!'); ?></p>
        <?php elseif ($serviceUseStatus == Team::SERVICE_USE_STATUS_READ_ONLY): ?>
            <p class="subscription-status-detail"><?= __('Your team can no longer create or edit content on Goalous. Your team account will be deactivated on %s', $this->TimeEx->formatYearDayI18nFromDate($team['service_use_state_end_date'])); ?></p>
        <?php else: //Deactivated ?>
            <p class="subscription-status-detail"><?= __('Your team no longer has access to  Goalous. Your team account will be deleted on %s', $this->TimeEx->formatYearDayI18nFromDate($team['service_use_state_end_date'])); ?></p>
        <?php endif; ?>
        ...
        <ol class="team-price-info-footer-legal">
            ...
            <li class="team-price-info-footer-legal-item"><?= __('Team members who fit the following criteria are considered to be billable monthly active members:'); ?>
                <ul class="team-price-info-footer-legal">
                    <li class="team-price-info-footer-legal-item"><?=__('Team members who are active by the payment date (those not deactivated by the team administrator)');?></li>
                    <li class="team-price-info-footer-legal-item"><?=__('In the event that team members were added by the team administrator between the current month’s payment date and 1 day prior to the following month’s payment date, the number of billable members will be more than the number of active members falling on the current month’s payment date. In addition, in that situation, added team member’s usage fee will be charged based on daily rate.');?></li>
                </ul>
            </li>
