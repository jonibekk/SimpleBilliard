 ...
 *
 */
class SetChargeTimingFlagShell extends AppShell
{
    ...
    function main()
    {
        try {
            ...
            if ($op == 'set' and (empty($value) or intval($value) < 0 or intval($value) > 3)){
            ...
            switch ($op) {
            case 'set':
                $res = $paymentKeyFlagClient->write($paymentFlagKey, intval($value));
                var_dump($res);
                break;
            case 'get':
