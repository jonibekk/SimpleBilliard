...
class TeamMember extends AppModel
{
    ...
    function saveNewMembersFromCsv($csv)
    {
        ...
        foreach ($this->csv_datas as $row_k => $row_v) {
            ...
            if (Hash::get($user['User'])) {
                ...
                $row_v['User']['language'] = Hash::get($row_v['LocalName']['language']) ? $row_v['LocalName']['language'] : 'eng';
    ...
    function validateNewMemberCsvData($csv_data)
    {
        ...
        foreach ($csv_data as $key => $row) {
            ...
            // for local name
            if (Hash::get($row['language']) && (Hash::get($row['local_first_name']) || Hash::get($row['local_last_name']))) {
                $this->csv_datas[$key]['LocalName']['language'] = $row['language'];
                $this->csv_datas[$key]['LocalName']['first_name'] = Hash::get($row['local_first_name']) ? $row['local_first_name'] : '';
                $this->csv_datas[$key]['LocalName']['last_name'] = Hash::get($row['local_last_name']) ? $row['local_last_name'] : '';
            } else {
    ...
    function getAllMembersCsvData($team_id = null)
    {
        ...
        foreach ($this->all_users as $k => $v) {
            ...
            $this->csv_datas[$k]['email'] = Hash::get($v['User']['PrimaryEmail']['email']) ? $v['User']['PrimaryEmail']['email'] : null;
            ...
            $this->csv_datas[$k]['member_no'] = Hash::get($v['TeamMember']['member_no']) ? $v['TeamMember']['member_no'] : null;
            $this->csv_datas[$k]['active_flg'] = Hash::get($v['TeamMember']['active_flg']) && $v['TeamMember']['active_flg'] ? 'ON' : 'OFF';
            $this->csv_datas[$k]['admin_flg'] = Hash::get($v['TeamMember']['admin_flg']) && $v['TeamMember']['admin_flg'] ? 'ON' : 'OFF';
            $this->csv_datas[$k]['evaluation_enable_flg'] = Hash::get($v['TeamMember']['evaluation_enable_flg']) && $v['TeamMember']['evaluation_enable_flg'] ? 'ON' : 'OFF';
            ...
            if (Hash::get($v['User']['MemberGroup'])) {
                foreach ($v['User']['MemberGroup'] as $g_k => $g_v) {
                    $key_index = $g_k + 1;
                    $this->csv_datas[$k]['group.' . $key_index] = Hash::get($g_v['Group']['name']) ? $g_v['Group']['name'] : null;
                }
            }
        }
    ...
    function setCoachNumberForCsvData($team_id)
    {
        foreach ($this->all_users as $k => $v) {
            ...
            $this->csv_datas[$k]['coach_member_no'] = Hash::get($coach_member['TeamMember']['member_no']) ? $coach_member['TeamMember']['member_no'] : null;
        }
        return;
    }
    ...
    function setUserInfoForCsvData()
    {
        foreach ($this->all_users as $k => $v) {
            ...
            $this->csv_datas[$k]['member_no'] = Hash::get($v['TeamMember']['member_no']) ? $v['TeamMember']['member_no'] : null;
            $this->csv_datas[$k]['member_type'] = Hash::get($v['MemberType']['name']) ? $v['MemberType']['name'] : null;
            $this->csv_datas[$k]['user_name'] = Hash::get($v['User']['display_username']) ? $v['User']['display_username'] : null;
            $this->csv_datas[$k]['coach_user_name'] = Hash::get($v['CoachUser']['display_username']) ? $v['CoachUser']['display_username'] : null;
        }
        return;
    }
