...
class Post extends AppModel
{
    ...
    {
        self::$TYPE_MESSAGE[self::TYPE_CREATE_GOAL] = __("あたらしいゴールをつくりました。");
        self::$TYPE_MESSAGE[self::TYPE_CREATE_CIRCLE] = __("あたらしいサークルをつくりました。");
    }
        ...
        //パラメータ指定あり
            ...
            if ($this->orgParams['circle_id']) {
                ...
                if (!$is_exists_circle || ($is_secret && !$is_belong_circle_member)) {
                    throw new RuntimeException(__("サークルが存在しないか、権限がありません。"));
                }
                $post_filter_conditions['OR'][] =
                    $db->expression('Post.id IN (' . $this->getSubQueryFilterMyCirclePostId($db, $start, $end,
            ...
            switch ($val['share_mode']) {
                case self::SHARE_PEOPLE:
                    if (count($val['PostShareUser']) == 1) {
                        $data[$key]['share_text'] = __("%sに共有",
                                                        $data[$key]['share_user_name']);
                    }
                    else {
                        $data[$key]['share_text'] = __('%1$sと他%2$s人に共有',
                                                        $data[$key]['share_user_name'],
                ...
                case self::SHARE_CIRCLE:
                    ...
                    if (count($val['PostShareUser']) == 0) {
                        if (count($val['PostShareCircle']) == 1) {
                            ...
                            $data[$key]['share_text'] = __('%1$s他%2$sサークルに共有',
                                                            $data[$key]['share_circle_name'],
                    ...
                    elseif (count($val['PostShareUser']) == 1) {
                        if (count($val['PostShareCircle']) == 1) {
                            $data[$key]['share_text'] = __('%1$sと%2$sに共有',
                                                            $data[$key]['share_circle_name'],
                                                            $data[$key]['share_user_name']);
                        }
                        else {
                            $data[$key]['share_text'] = __('%1$sと%2$s他%3$sサークルに共有',
                                                            $data[$key]['share_user_name'],
                                                            $data[$key]['share_circle_name'],
                                                            count($val['PostShareCircle']) - 1);
                        }
                    ...
                    //共有ユーザが２人以上いる場合
                        ...
                        if (count($val['PostShareCircle']) == 1) {
                            $data[$key]['share_text'] = __('%1$s,%2$sと他%3$s人に共有',
                                                            $data[$key]['share_circle_name'],
                                                            $data[$key]['share_user_name'],
                                                            count($val['PostShareUser']) - 1);
                        }
                        else {
                            $data[$key]['share_text'] = __('%1$sと他%2$s人,%3$s他%4$sサークルに共有',
                                                            $data[$key]['share_user_name'],
                                                            count($val['PostShareUser']) - 1,
                                                            $data[$key]['share_circle_name'],
                                                            count($val['PostShareCircle']) - 1);
