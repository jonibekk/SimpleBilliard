...
class UsersController extends AppController
{
    ...
    public function login()
    {
        ...
        if ($is_account_locked) {
            $this->Noty->outError(__("Your account is tempolary locked. It will be unlocked after %s mins.",
                ACCOUNT_LOCK_TTL / 60));
            return $this->render();
        }
        ...
        if (!$user_info) {
            $this->Noty->outError(__("Email address or Password is incorrect."));
            return $this->render();
        }
    ...
    function two_fa_auth()
    {
        ...
        if (!$is_avail_auth) {
            $this->Noty->outError(__("Error. Try to login again."));
        ...
        if ((empty($this->Session->read('2fa_secret')) === false && empty($this->request->data['User']['two_fa_code']) === false)
            ...
            $this->Noty->outError(__("Incorrect 2fa code."));
            return $this->render();
        }
    }
    ...
    function two_fa_auth_recovery()
    {
        ...
        if (!$row) {
            $this->Noty->outError(__("Incorrect recovery code."));
            return $this->render();
        }
        ...
        if (!$res) {
            $this->Noty->outError(__("An error has occurred."));
            return $this->render();
        }
    ...
    function _afterAuthSessionStore()
    {
        ...
        if ($this->Auth->login()) {
            ...
            $this->_refreshAuth();
            $this->_setAfterLogin();
            $this->Noty->outSuccess(__("Hello %s.", $this->Auth->user('display_username')),
    ...
    public function logout()
    {
        ...
        $this->Cookie->destroy();
        $this->Noty->outInfo(__("See you %s", $user['display_username']),
            ['title' => __("Logged out")]);
    ...
    public function register_with_invite()
    {
        ...
        try {
            ...
            $this->Noty->outError(__("The invitation token is incorrect. Check your email again."));
            return $this->redirect('/');
        }
        ...
        if ($step === 1) {
            ...
            if ($this->User->validates($this->request->data)) {
                ...
                //エラーメッセージ
                $this->Noty->outError(__('Failed to save data.'));
                return $this->render($profileTemplate);
            }
        }
        //パスワード入力画面の場合
        ...
        if ($invitedTeam === false) {
            ...
            $this->Noty->outError(__("Failed to register user. Please try again later."));
            return $this->redirect("/");
        }
    ...
    public function verify($token = null)
    {
        try {
            ...
            if (!$last_login) {
                ...
                $this->Noty->outSuccess(__('Succeeded to register!'));
                //新規ユーザ登録時のフロー
                $this->Session->write('add_new_mode', MODE_NEW_PROFILE);
                ...
                //ログインされていれば、メール追加
                $this->Noty->outSuccess(__('Authenticated your email address.'));
                /** @noinspection PhpInconsistentReturnPointsInspection */
                /** @noinspection PhpVoidFunctionResultUsedInspection */
                return $this->redirect('/');
            }
    ...
    public function change_email_verify($token)
    {
        ...
        $this->Noty->outSuccess(__("Email address is changed."));
        /** @noinspection PhpVoidFunctionResultUsedInspection */
        return $this->redirect(['action' => 'settings']);
    }
    ...
    public function password_reset($token = null)
    {
        ...
        if (!$token) {
            ...
            if ($user) {
                ...
                $this->Noty->outSuccess(__("Password reset email has been sent. Please check your email."),
                    ['title' => __("Email sent.")]);
            }
            return $this->render('password_reset_request');
        }
        ...
        if (!$user_email) {
            $this->Noty->outError(__("Password code incorrect. The validity period may have expired. Please resend email again."),
        ...
        if ($successPasswordReset) {
            ...
            $this->Noty->outSuccess(__("Please login with your new password."),
    ...
    public function token_resend()
    {
        ...
        if ($this->request->is('post') && !empty($this->request->data)) {
            ...
            if ($email_user = $this->User->saveEmailToken($this->request->data['User']['email'])) {
                ...
                $this->Noty->outSuccess(__("Confirmation has been sent to your email address."),
    ...
    public function settings()
    {
        ...
        if ($this->request->is('put')) {
            ...
            if ($this->User->saveAll($this->request->data)) {
                ...
                $this->Noty->outSuccess(__("Saved user setting."));
                $this->redirect('/users/settings');
            } else {
                $this->Noty->outError(__("Failed to save user setting."));
            }
            $me = $this->_getMyUserDataForSetting();
    ...
    public function change_password()
    {
        ...
        try {
            ...
            $this->Noty->outError($e->getMessage(), ['title' => __("Failed to save password change.")]);
            /** @noinspection PhpVoidFunctionResultUsedInspection */
            return $this->redirect($this->referer());
        }
        $this->Noty->outSuccess(__("Changed password."));
    ...
    public function change_email()
    {
        ...
        $this->Noty->outInfo(__("Confirmation has been sent to your email address."));
    ...
    public function accept_invite($token)
    {
        ...
        if (!$this->Invite->isForMe($token, $this->Auth->user('id'))) {
            $this->Noty->outError(__("This invitation isn't not for you."));
            return $this->redirect("/");
        }
        ...
        if ($invitedTeam === false) {
            ...
            $this->Noty->outError(__("Failed to join team. Please try again later."));
            return $this->redirect("/");
        }

        $this->Session->write('referer_status', REFERER_STATUS_INVITED_USER_EXIST);
        $this->Noty->outSuccess(__("Joined %s.", $invitedTeam['Team']['name']));
        return $this->redirect("/");
    }
    ...
    function register_2fa()
    {
        ...
        $this->Noty->outSuccess(__("Succeeded to save 2-Step Verification."));
        $this->Flash->set(null,
    ...
    function delete_2fa()
    {
        ...
        $this->Noty->outSuccess(__("Succeeded to cancel 2-Step Verification."));
        return $this->redirect($this->referer());
    }
    ...
    function add_subscribe_email()
    {
        ...
        if (!$SubscribeEmail->save($this->request->data)) {
            $this->Noty->outError($SubscribeEmail->validationErrors['email'][0]);
            return $this->redirect($this->referer());
        }
        $this->Noty->outSuccess(__('Registered email address.'));
        return $this->redirect($this->referer());
    }
