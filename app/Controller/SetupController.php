...
class SetupController extends AppController
{
    ...
    public function ajax_create_goal()
    {
        ...
        if ($res) {
            $this->Noty->outSuccess($msg = __("Created a goal."));
            $error = false;
        } else {
    ...
    public function ajax_create_circle()
    {
        ...
        if (!$CircleService->validateCreate($data, $userId)) {
            $this->Noty->outError($validateCreate);
    ...
    public function ajax_join_circle()
    {
        ...
        $this->Noty->outSuccess(__("Join a circle."));
    ...
    public function ajax_add_action()
    {
        ...
        // push
        $this->Noty->outSuccess($msg = __("Added an action."));
        //セットアップガイドステータスの更新
        $this->updateSetupStatusIfNotCompleted();
        $res = [
            'error' => false,
