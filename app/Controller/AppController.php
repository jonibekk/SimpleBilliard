...
//    ];
    ...
    public function beforeFilter()
    {
        ...
        if (ENV_NAME == "www") {
            $this->title_for_layout = __('Goalous(ゴーラス)');
        }
        else {
            $this->title_for_layout = "[" . ENV_NAME . "]" . __('Goalous(ゴーラス)');
        }
        ...
        if ($this->Auth->user()) {
            ...
            if (!$this->request->is('ajax')) {
                ...
                if ($this->current_team_id && !isset($active_team_list[$this->current_team_id])) {
                    ...
                    $this->Pnotify->outError(__("ログイン中のチームが削除されたため、ログアウトされました。"));
                    $this->Auth->logout();
                    return;
                }
    ...
    public function _switchTeamBeforeCheck()
    {
        ...
        if ($current_team_id != $request_team_id) {
            ...
            if (!array_key_exists($request_team_id, $team_list)) {
                ...
                $this->Pnotify->outError(__("このチームへのアクセス権限がありません。"));
                $this->redirect('/');
            }
            else {
                //チームを切り替え
    ...
    public function _setViewValOnRightColumn()
    {
        ...
        if ($cached_my_goal_area_vals !== false) {
            ...
            $goal_list_for_action_option = [null => __('ゴールを選択する')] + $current_term_goals_name_list;
