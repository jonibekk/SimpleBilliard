...
{
    public function upload(\SplFileInfo $fileVideo, array $user, int $teamId): array
    {
        /** @var Video $Video */
        ...
        $Video->create([
            ...
            'hash' => hash_file('sha256', $fileVideo->getRealPath()),//$request->getFileHash(),// TODO: move to some function/class
            'file_size' => filesize($fileVideo->getRealPath()),
            'file_name' => $fileVideo->getFilename(),// TODO: passing wrong name(pass the user's file name)
            'resource_path' => null,//$result->getResourcePath(),
        ]);
        $video = $Video->save();
        $video = reset($video);
        $VideoStream->create([
        ...
        $request = new VideoUploadRequestOnPost($fileVideo, $user, $teamId, $video, $videoStream);
