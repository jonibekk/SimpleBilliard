...
App::import('Component', 'NotifyBizComponent');
...
class GoalApprovalService extends AppService
{
    ...
    function sendCommentNotify($goalMemberId, $myUserId, $commentId)
    {
        ...
        if($goalMemberUserId == $myUserId) {
            ...
            $toUserList[] = $coachUserId;
        // コーチの場合、コーチーに通知
        } else {
            $toUserList[] = $goalMemberUserId;
        }

        /** @var NotifyBizComponent $NotifyBizComponent */
        $NotifyBizComponent = ClassRegistry::init("NotifyBizComponent");
        return $NotifyBizComponent->execSendNotify(NotifySetting::TYPE_APPROVAL_COMMENT, $goalMemberId, $commentId, $toUserList);
    }
