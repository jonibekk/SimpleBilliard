...
class UploadServiceTest extends GoalousTestCase
{
    ...
    public function test_readFromBuffer_success()
    {
        ...
        $UploadedFile = new UploadedFile($this->encodedFile);

        $key = $UploadService->buffer(2, 1, $this->encodedFile);

        $returnData = $UploadService->read(2, 1, $key);

        $this->assertEquals($UploadedFile->getFile(), $returnData->getFile());
        $this->assertEquals($UploadedFile->getFileType(), $returnData->getFileType());
    ...
    public function test_countBuffer_success()
    {
        ...
        $UploadService->buffer(6, 1, $this->encodedFile);
        $UploadService->buffer(6, 1, $this->encodedFile);
        $UploadService->buffer(6, 1, $this->encodedFile);

        $UploadService->buffer(7, 1, $this->encodedFile);
    ...
    public function test_deleteFileFromBuffer_success()
    {
        ...
        $uuid1 = $UploadService->buffer(4, 1, $this->encodedFile);
        $uuid2 = $UploadService->buffer(5, 1, $this->encodedFile);
    ...
    public function test_deleteAllFileFromBuffer_success()
    {
        ...
        $uuid1 = $UploadService->buffer(3, 1, $this->encodedFile);
        $uuid2 = $UploadService->buffer(3, 1, $this->encodedFile);
    ...
    public function test_autoDeleteOverLimit_success()
    {
        ...
        $uuid1 = $UploadService->buffer(8, 1, $this->encodedFile);
        sleep(2);
        $uuid2 = $UploadService->buffer(8, 1, $this->encodedFile);
        sleep(2);
        $uuid3 = $UploadService->buffer(8, 1, $this->encodedFile);
        sleep(2);

        while ($count++ < 20) {
            $UploadService->buffer(8, 1, $this->encodedFile);
        }
