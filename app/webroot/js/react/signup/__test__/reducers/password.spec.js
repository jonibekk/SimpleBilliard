...
describe('reducer::password', () => {
  ...
  it('PASSWORD_IS_INVALID', () => {
    ...
    const newState = passwordReducer({}, { type: types.PASSWORD_IS_INVALID, password_is_invalid: true });
    expect(newState).toInclude(expect_value)
  })
  it('CHECKING_PASSWORD', () => {
    const expect_value = { checking_password: true }
    const newState = passwordReducer({}, { type: types.CHECKING_PASSWORD, checking_password: true });
    expect(newState).toInclude(expect_value)
  })
})
