...
export default class GoalCard extends React.Component {
  ...
  render() {
    ...
    const { goal, top_key_result, goal_changed_columns, tkr_changed_columns } = this.props
      ...
      <div className="goals-approval-detail-goal-card" key={ goal.modified + top_key_result.modified }>
          <div className="goals-approval-detail-table">
              ...
              <div className="goals-approval-detail-goal-card-info">
                  <p><i className="fa fa-folder-o" aria-hidden="true"></i> <span className={`${goal_changed_columns['goal_category_id'] && 'changed'}`}>{ goal.goal_category.name }</span></p>
                  <p><a href={`/goals/view_info/goal_id:${goal.id}`} className={`goals-approval-detail-goal-card-info-link ${goal_changed_columns['name'] ? 'changed' : ''}`} target={cake.is_mb_app ? "_self" : "_blank"}>{ goal.name }</a></p>
